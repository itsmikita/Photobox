...
$('#photobox .photobox-wrapper').append($('<div />').addClass('photobox-placeholder'));this.controls();this.keys();this.touchable();$('.photobox-wrapper').bind('tap click',function(e){PB.toggle_meta();});},controls:function(){var PB=this;$('#photobox .photobox-button-prev').click(function(e){e.preventDefault();PB.prev();});$('#photobox .photobox-button-next').click(function(e){e.preventDefault();PB.next();});$('#photobox .photobox-button-close').click(function(e){e.preventDefault();PB.close();});},keys:function(){var PB=this;$(document).keydown(function(e){PB.fadeout_meta();switch(e.keyCode){case 37:PB.prev();break;case 39:PB.next();break;case 27:PB.close();break;}});},touchable:function(){var PB=this,offsetX=0,startX=0,width=$(window).width();$('#photobox .photobox-wrapper').bind('touchstart',function(e){$(this).removeClass('animate');offsetX=$(this).position().left;startX=e.originalEvent.touches[0].pageX;}).bind('touchmove',function(e){e.preventDefault();PB.fadeout_meta();$(this).css('left',(offsetX*-1+(startX-e.originalEvent.touches[0].pageX))*-1+'px');}).bind('touchend',function(){var n=Math.round($(this).position().left/width);if(n>0)
n=0;if(n<this.items.length-1)
n=this.items.length-1;PB.current=n;PB.load(n);$(this).addClass('animate').css('left',n*-100+'%');});$('#photobox .photobox-placeholder img').bind('gesturechange',function(e){var scale=e.originalEvent.scale;if(0.5>scale)
scale=0.5;$(this).css('-webkit-transform','scale( '+scale+' )');});},open:function(n){this.setup();this.current=n;var offset=this.current*100*-1;$('#photobox .photobox-wrapper').css('left',offset+'%');this.load(n);$('#photobox').animate({opacity:1},100);},close:function(){this.cleanup();},prev:function(){this.current--;if(0>this.current)
